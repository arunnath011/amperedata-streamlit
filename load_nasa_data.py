...
def create_tables(conn):
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_resistance_battery ON resistance_data(battery_id)")
...
def load_nasa_data(data_dir: Path, db_path: Path):
    ...
    for battery_id in batteries:
        ...
        if not discharge_records.empty:
            ...
            initial_capacity = discharge_records['Capacity'].iloc[0] if pd.notna(discharge_records['Capacity'].iloc[0]) else None
            final_capacity = discharge_records['Capacity'].iloc[-1] if pd.notna(discharge_records['Capacity'].iloc[-1]) else None
            ...
            for idx, row in discharge_records.iterrows():
                if pd.notna(row.get('Capacity')):
                    retention = (row['Capacity'] / initial_capacity * 100) if initial_capacity else None
                    cursor.execute("""
                        INSERT INTO capacity_fade (battery_id, cycle_number, capacity_ah, retention_percent, timestamp)
                        VALUES (?, ?, ?, ?, ?)
                    """, (battery_id, cycle_num, row['Capacity'], retention, str(row.get('start_time', ''))))
                    cycle_num += 1
            ...
            for idx, row in impedance_records.iterrows():
                if pd.notna(row.get('Re')) or pd.notna(row.get('Rct')):
                    cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?)
                    """, (battery_id, row.get('Re'), row.get('Rct'), str(row.get('start_time', '')), row.get('test_id')))
        ...
        for idx, row in tqdm(battery_meta.iterrows(), total=len(battery_meta), desc=f"  {battery_id}"):
                ...
                try:
                    ...
                    for _, cycle_row in sampled_df.iterrows():
                        ...
                        """, (
                            ...
                            cycle_row.get('Capacity_Ah') if 'Capacity_Ah' in cycle_df.columns else None,
                            cycle_row.get('Temperature_measured') if 'Temperature_measured' in cycle_df.columns else None
                        ))
